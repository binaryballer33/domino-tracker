...
/**
 * Returns an array of 7 arrays, each row containing the dominos sorted in order for example:
 ...
 *  ...
 * ]
 * @param dominos - The array of dominos to sort
 * @returns An array of 7 arrays, each containing the dominos sorted in order
 */
export default function getSevenRowsOfSortedDominos(dominos: DominoWithIndex[]): DominoWithIndex[][] {
    return Array(7)
        .fill(null)
        .map((_, eyeValue) => {
            // Find all dominos that contain this eye value
            const matchingDominos = dominos.filter(
                (domino) => domino.firstEye === eyeValue || domino.secondEye === eyeValue,
            )

            // Sort them by the other value (or by the same value for doubles)
            return matchingDominos.sort((a, b) => {
